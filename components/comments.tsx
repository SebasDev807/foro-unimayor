...
const CommentsPage: React.FC = () => {
    ...
    const post = postString ? JSON.parse(decodeURIComponent(postString)) : null;
    ...
    const currentUser = 'Usuario1'; // Make the logic in the backend so that this is the value of the user making the post.
            ...
            .map(comment => {
                ...
                return (
                    <div key={comment.id} className="relative mb-4 pl-4">
                        <div className={`bg-white border border-gray-200 rounded-lg p-4 ${level > 0 ? 'ml-4 border-l-2 border-gray-300' : ''}`}>
                            <div className="flex items-center mb-3">
                                <img src={comment.avatar} alt="Avatar" className="w-10 h-10 rounded-full mr-3" />
                                <div className="flex-1">
                                    <div className="flex items-center">
                                        <div className="font-bold">{comment.username}</div>
                                        <span className="ml-2 text-sm py-1 px-2 rounded bg-gray-100 border-gray-500 text-gray-700">Comentario</span>
                                    </div>
                            ...
                            <div className="flex space-x-4 mt-3">
                                <div className="flex items-center space-x-1 cursor-pointer" onClick={() => handleLike(comment.id)}>
                                    <HeartIcon
                                        className={`h-6 w-6 transition-transform duration-300 ${comment.liked ? 'text-red-500 scale-125' : 'text-gray-500'}`}
                                        style={{ fill: comment.liked ? 'red' : 'none' }}
                                    />
                                    <span className="text-sm">{comment.likes}</span>
                                </div>
                                <div className="flex items-center space-x-1 cursor-pointer" onClick={() => handleReplyClick(comment.id)}>
